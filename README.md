...
The cache controller is driven by an 8-state Mealy Finite State Machine (FSM) that resolves hits in a single cycle and manages complex bus transfers for misses, scratch accesses, and dirty block replacements. A secondary 2-state FSM handles hardware-level memory errors.

* **Cache Specs:** 2-way set-associative, 4 sets, 4 words per block (32-bit words).
* **Replacement Policy:** FIFO.
* **Write Policies:** Write-back (copyback) for dirty blocks; Write-around for write misses (direct to Main Memory without loading the block to cache).
